...
class char():
    ...
    def effecting(self,obj,effect,effecting,round=1,forever=False):
        if forever == True:
                round = 9999999999999999999999999999
        ...
        if effect == "poisoning":
            obj.attack(obj,effecting,nomsg=True)
            print(F"{obj.name}受到{effecting}傷害。")
            obj.effect[effect] = [effect,effecting,round]

 
    def noeffect(self,char,effect):
        effecting = char.effect[effect]
        try:
            if effect == "atkadd":
                self.atk -= effecting[effect]
            del effecting
...
chardeadman = deadman("死亡之身",125,15,["daed","man","relive"],sk["死亡之身"],12)
charhellwife = hellwife("地獄女僕",100,35,["live in hell"],sk["地獄女僕"],10)
charboneman = boneman("零骨",90,20,["bone"],sk["零骨"],12)
charbeliever = believer("\"使徒\"",50,10,["group","relive"],sk["\"使徒\""],6)
charbrokewheel = brokewheel("破輪",90,5,["wheel"],sk["破輪"],9)
 
 
 
 
playerlist = [chardeadman,charhellwife,charboneman,charbeliever,charbrokewheel]
    ...
    for player in playerlist:
            ...
            try:
                for effection in player.effect:
                    effect = player.effect[effection]
                    effecttime = deepcopy(effect[2])
                    effecting = effect[1]
                    effectname = effect[0]
                    ...
                    if effecttime > 0:
                        ...
                        effect[2] = deepcopy(effecttime)
                    else:
                        playchar.noeffect(effectname)
            except:
                pass
